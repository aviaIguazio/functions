...
def test_code_to_function_sklearn_classifier():
    acquire_run = generate_data()
    fn = code_to_function(name='test_sklearn',
                          filename="sklearn_classifier.py",
                          handler="train_model",
                          kind="local",
                          )
    # define model
    params = {
            ...
            "CLASS_n_jobs": -1,
            "CLASS_oob_score": True}
    fn.spec.command = "sklearn_classifier.py"
    train_run = fn.run(params=params, inputs={"dataset": acquire_run.outputs["price"]}
                       #,local=True,
                       ,artifact_path="artifacts")
    assert Path(CLASSIFIER_PATH).is_file()